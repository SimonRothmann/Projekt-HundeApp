...
public static class DemoDataSeeder
{
    ...
    public static async Task SeedAsync(IServiceProvider services)
    {
        ...
        if (await userManager.FindByEmailAsync("trainer@canistrack.test") is not null)
            return;

        var admin = await CreateUserAsync(userManager, "admin@canistrack.test", "Admin", "Muster", [Roles.User, Roles.Admin]);
        var trainer = await CreateUserAsync(userManager, "trainer@canistrack.test", "Anna", "Trainer", [Roles.User]);
        var member1 = await CreateUserAsync(userManager, "mitglied1@canistrack.test", "Max", "Mustermann", [Roles.User]);
        var member2 = await CreateUserAsync(userManager, "mitglied2@canistrack.test", "Lisa", "Beispiel", [Roles.User]);
